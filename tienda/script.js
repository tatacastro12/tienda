...
document.addEventListener("DOMContentLoaded", () => {
    ...
    cars.forEach((car) => {
      car.addEventListener("click", () => {
        let nombreProducto =
          car.parentNode.querySelector(".top_name").textContent; // Obtiene el nombre del producto
        let precioProducto = parseFloat(
          car.parentNode.querySelector(".price").textContent
        ); // Obtiene el precio del producto
        let cantidadProducto = car.parentNode.querySelector(
          'input[type="number"]'
        ).value; // Obtiene la cantidad del producto ingresada por el usuario
    ...
    function actualizarBotones() {
      ...
      botonesEliminar.forEach((boton) => {
        boton.addEventListener("click", () => {
          ...
          if (item.cantidad > 0) {
            // Verifica que la cantidad del producto sea mayor a 0
    ...
    send.addEventListener("click", () => {
      ...
      info.forEach((item) => {
        mensaje += `${item.cantidad} x ${item.nombre} (${item.precio.toFixed(
          2
      ...
      let url = `https://wa.me/${numeroWhatsApp}?text=${encodeURIComponent(
        mensaje
      )}`; // Crea la URL para enviar el mensaje a WhatsApp
      window.open(url, "_blank"); // Abre la URL en una nueva pestaña
      alert(`Pedido enviado a WhatsApp: \n${mensaje}`); // Alerta para confirmar que se envió el pedido
    });
  });
